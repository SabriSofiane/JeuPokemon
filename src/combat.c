...
int init_combat(motor_t ** motor)
{
  ...
  t_pkm * wild_pkm  = malloc(sizeof(t_pkm));
*/
  /*
  t_pkm pkm;
  t_pkm wild_pkm;
...
*/
  ...
  init_pkm_enemy(&(*motor)->wild_pkm);
  init_pkm_team(&(*motor)->pkm, "1.txt");
  printf("lol\n");

  push(&(*motor)->player->liste_player, (*motor)->pkm);
  //insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);
  printf("lol2\n");
  //push(&(*motor)->player->liste_player, (*motor)->pkm);
  //init_pkm_team(&(*motor)->pkm, "./src/asset/stat_pkm/2.txt");
  /*
  insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);

  init_pkm_team(&(*motor)->pkm, "../src/asset/stat_pkm/3.txt");
  insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);
  init_pkm_team(&(*motor)->pkm, "../src/asset/stat_pkm/7.txt");
  insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);
  init_pkm_team(&(*motor)->pkm, "../src/asset/stat_pkm/5.txt");
  insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);
  init_pkm_team(&(*motor)->pkm, "../src/asset/stat_pkm/6.txt");
  insertAtEnd((*motor)->player->liste_player, (*motor)->pkm);
  */
}
...
void swapNodes(motor_t ** motor, int n1, int n2){
    Liste_t *prevNode1 = NULL, *prevNode2 = NULL, *node1 = (*motor)->player->liste_player, *node2 = (*motor)->player->liste_player, *temp = NULL;
    int tp;
    //Checks if list is empty
    if((*motor)->player->liste_player == NULL) {
        return;
    }
